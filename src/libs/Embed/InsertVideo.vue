<template>
    <button type="button" class="btn-toggle" title="Video URL" @click="addEmbed">
    ...
    methods: {
        ...
        renderEmbed(url, elm) {
            ...
            setTimeout(() => {
                ...
                // const currentIframe = focused.querySelector('br')
        ...
        addHttpIfRequired(url) {
            const httpRegex = /^https?:\/\//;
            return httpRegex.test(url) ? url : `https://${url}`;
        },

        extractAndValidateVideoUrl(linkUrl) {
            ...
            if (
                !Object.keys(this.whiteEmbedList).some((element) => videoSrc.includes(element))
            ) {
                return '';
            }

            return videoSrc.includes('embed') ? videoSrc : this.transformToPlayer(videoSrc);
        },

        transformToPlayer(link) {
            if (link.includes('youtube')) {
                ...
                return `${this.whiteEmbedList.youtube}/embed/${matched[1].includes('&') ? matched[1].split('&')[0] : matched[1]}`;
            }
